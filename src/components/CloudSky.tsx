...
export type Clouds = { softness?: number; shadow?: number; cirrus?: number };
export type Sun = { x?: number; y?: number; glow?: string };
export type Pointer = { parallax?: number; wind?: number; damping?: number };
...
export function CloudSky(props: CloudSkyProps) {
  const {
    style,
    className,
    background = "#0075FF",
  ...
  const ptrRef = useRef({ x: 0, y: 0, inside: true });
  ...
  useEffect(() => {
    ...
    // Track on window so mouse movement anywhere on the page affects the sky parallax and wind!
    const track = (e: MouseEvent | PointerEvent) => {
      const w = window.innerWidth || 1;
      const h = window.innerHeight || 1;
      ptrRef.current.x = (e.clientX / w) * 2 - 1;
      ptrRef.current.y = 1 - (e.clientY / h) * 2;
...
export default function CloudSkyDefault(props: CloudSkyProps) {
  return <CloudSky {...__originkitPresetProps} {...props} />;
}
